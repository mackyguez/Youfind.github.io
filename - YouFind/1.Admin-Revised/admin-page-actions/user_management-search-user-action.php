<?php 
	$connect = new PDO('mysql:host=localhost;dbname=youfind_database;', 'root', '');
		...
		if(isset($_POST['query'])) {
			$output = '';
			$recieved_query = $_POST['query'];
			$pick_search = $_POST['pick_search'];
				...
				if($pick_search == 'All') {
					...
					$query = '	SELECT * FROM youfind_user WHERE email LIKE "%":email"%"';
					$statement = $connect->prepare($query);
					$statement->execute(array(':email'	=> $recieved_query));
						...
						if(@$row['user_type'] == 'Customer') {
							...
							<tr height="50px" style="border-bottom: 1px solid black;">
								...
								<td style="padding: 10px;">
									...
									<input type="button" id="view_files_id" class="btn-default view-files_customer" value="View Files" data-user_view_files_id="'.$row['user_id'].'" data-user_fname="'.$row['first_name'].'">
								</td>
							...
							$output .= '		
								<td style="padding: 10px;">
									...
									<input type="button" id="view_files_id" class="btn-default view-files_seller" value="View Files"data-user_view_files_id="'.$row['user_id'].'" data-user_fname="'.$row['first_name'].'" data-user_id_front="'.$row['front_id'].'" data-user_id_front="'.$row['back_id'].'">
								</td>';
				...
				if($pick_search == 'Customer') {
					...
					$query = '	SELECT * FROM youfind_user WHERE email LIKE "%":email"%" AND user_type = "Customer"';
					$statement = $connect->prepare($query);
					$statement->execute(array(':email'	=> $recieved_query));
					$result = $statement->fetchAll();
					foreach($result as $row) {
				...
				if($pick_search == 'Verified - Entrepreneurs') {
					$output = '
						<tr style="font-weight: bold; height: 30px; background-color: #f3f4d1;">
					...
					';
					$query = '	SELECT * FROM youfind_user WHERE email LIKE "%":email"%" AND user_type = "Seller" AND user_status = "verified"';
					$statement = $connect->prepare($query);
					$statement->execute(
						array(
							':email'	=> $recieved_query
						)
					);
					$result = $statement->fetchAll();
					foreach($result as $row) {
					...
					echo $output;
				}//Pick search Verified Seller


				if($pick_search == 'Unverified - Entrepreneurs') {
					$output = '
						<tr style="font-weight: bold; height: 30px; background-color: #f3f4d1;">
							<td class="td-header" width="50.75px"><input type="checkbox" name="input_checkbox" id="input_checkbox"></td>
							<td class="td-header fname" width="152.25px">First Name</td>
					...
					';
					$query = '	SELECT * FROM youfind_user WHERE email LIKE "%":email"%" AND user_type = "Seller" AND user_status = "not verified"';
					$statement = $connect->prepare($query);
					$statement->execute(
						array(
							':email'	=> $recieved_query
					...
					echo $output;
				}//Pick search Unverified Seller

				
				if($pick_search == 'Banned') {
					$output = '
						<tr style="font-weight: bold; height: 30px; background-color: #f3f4d1;">
							<td class="td-header" width="50.75px"><input type="checkbox" name="input_checkbox" id="input_checkbox"></td>
							<td class="td-header fname" width="152.25px">First Name</td>
					...
					';
					$query = '	SELECT * FROM youfind_user WHERE email LIKE "%":email"%" AND user_type = "Seller" AND user_status = "banned"';
					$statement = $connect->prepare($query);
					$statement->execute(
						array(
							':email'	=> $recieved_query
