<?php 
		...
		$query = 'SELECT * FROM index_newsfeed_tbl ORDER BY product_id DESC LIMIT 4';
		$statement = $connect->prepare($query);
		$statement->execute();
		$resultIndexProduct = $statement->fetchAll();
		$resultCount = $statement->rowCount();

		$cartFetch = 'SELECT * FROM index_newsfeed_tbl';
		...
		if(@$_POST['action'] == 'load_products_limit') { 
		  		for($i=0; $i<$resultCount; $i++) { ?>
						<div class="product-fetch" id="product_fetch">
							<div class="prod-image-holder" id="product_id" data-image_product_id="<?php echo @$resultIndexProduct[$i]['product_id']; ?>" style="cursor: pointer;">
								<img src="product-photo/<?php echo @$resultIndexProduct[$i]['product_photo']; ?>" width="100%" height="100%">
							</div>
							<div class="prod-name"><strong><?php echo @$resultIndexProduct[$i]['product_name']; ?></strong></div>
							<div class="prod-seller">By: <a class="seller-name"><?php echo @$resultIndexProduct[$i]['shopname']; ?></a></div>
							<img src="images/verified.png" class="verified-img">
							<p class="prod-price">&#8369; <?php echo @number_format($resultIndexProduct[$i]['product_price'],2); ?></p>
					...
					<?php if(isset($_SESSION['customer_user_id'])) { ?>
								<div class="add-to-cart-holder" id="add_to_cart"
									data-product_id="<?php echo @$resultIndexProduct[$i]['product_id']; ?>" 
									data-product_name="<?php echo @$resultIndexProduct[$i]['product_name']; ?>"
									data-product_price="<?php echo @$resultIndexProduct[$i]['product_price']; ?>"
									data-product_seller_id="<?php echo @$resultIndexProduct[$i]['user_id']; ?>"
									data-product_seller_shopname="<?php echo @$resultIndexProduct[$i]['shopname']; ?>"
									data-product_photo="<?php echo @$resultIndexProduct[$i]['product_photo']; ?>"
									data-product_quantity="1"
									data-product_limit="<?php echo @$resultIndexProduct[$i]['product_quantity']; ?>">
			...
			<?php  	} ?>
<?php 	} 
		if(@$_POST['action'] == 'load_all_products') { ?>
