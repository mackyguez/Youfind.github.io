<?php 
	...
	$result1 = $statement1->fetchAll();
		...
		if($result1[0]['youwallet'] > $_SESSION['total_amount']) {
			if(@$_POST['action'] == 'youwallet_transaction') {
				...
				$deduct = 'SELECT * FROM youfind_user WHERE user_id = '.$_SESSION['customer_user_id'].'';
				$deductStatement = $connect->prepare($deduct);
				$deductStatement->execute();
				$deductResult = $deductStatement->fetchAll();

				if($_SESSION['delivery_option'] == 'standard') {
					$compute = ($deductResult[0]['youwallet'] - $_SESSION['total_amount']);
				}
				else if($_SESSION['delivery_option'] == 'express') {
					$compute = ($deductResult[0]['youwallet'] - ($_SESSION['total_amount']+29));
				}
				$updateYW = 'UPDATE youfind_user SET youwallet = '.$compute.' WHERE user_id = '.$_SESSION['customer_user_id'].'';
				$updateYWStatement = $connect->prepare($updateYW);
				$updateYWStatement->execute();
				echo 'Done';
	 		}
		}
		else {
